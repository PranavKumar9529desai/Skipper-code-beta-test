...
class Skipera {
    ...
    async read_item_with_new_req(item_id){
        const jsonDataObject = `{\"userId\":${this.user_id},\"courseId\":\"${this.course_id}\",\"itemId\":\"${item_id}\"}`

        const config = {
            method: 'post',
            url: `${this.base_url}/onDemandSupplementCompletions.v1`,
            headers: headersAndCookies,
            data: jsonDataObject
        };
        ...
        axios.request(config)
            .then((response) => {
                ...
                logger.error("Some error occured while reading.",error.message)
            });
    }
}
    program
